      ...
      <section  v-loading="loading" :element-loading-text="$t('m.loading')">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="180px"  size="small">
          ...
          <el-form-item :label="$t('m.authority.userGroup')" prop="userGroup">
            <el-select v-model="ruleForm.userGroup" id="createuser_usergroup" clearable :placeholder="$t('m.rules.pleaseChoose')+$t('m.authority.userGroup')">
               <el-option
                v-for="item in group"
                :key="item.value"
                :label="item.label"
          ...
          <el-form-item :label="$t('m.store.dataVolume')" v-if="isVisibleVolumes">
            ...
            <span v-if="volumeList[0].volumeName==='nfs' && volumeList[0].status==='Started'">
              nfs
            </span>
