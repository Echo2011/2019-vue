      ...
      <section class="show-info viewer">
        ...
        <div class="bread-road">
          <p v-show="dirList.length>=2">
            <span id="storelist_viewergetdata" @click="viewerGetData('',dirList.length-2)">{{$t('m.store.BackTo')}}</span><i>｜</i> <span  v-for="(item,index) in dirList" :key="index" @click="viewerGetData('',index)"><i v-if="index !=0"> &gt; </i>{{item}}</span>
          </p>
        ...
        <table v-loading="loading" :element-loading-text="$t('m.loading')">
           ...
           <tbody v-if="list.length>0" class="tbody-viewer" >
             <tr v-for="(item,index) in list" :key="index" >
               <td><el-checkbox v-model="item.isCheck" @change="handleItemCheckedChange(item.fileName,item.isCheck)"></el-checkbox></td>
               <td class="ellipsis" v-loading="item.isUnzipingLoading" :element-loading-text="$t('m.loading')">
                 <!-- 图片 -->
                 <span v-if="item.imgUrl" @click="dialogViewFileVisible=true;imgVisible=true;viewFile(item,true)">
                 <!-- <span v-if="item.imgUrl" @click="dialogViewFileVisible=true;imgVisible=true;imgUrl=item.imgUrl"> -->
                    <i class="el-icon-picture-outline"></i>
                    {{item.fileName}}
                  </span>
                  <!-- 文件夹 -->
                  <span class="clickable" v-else-if="item.directory" @click="viewerGetData(item.fileName)">
                    <svg class="icon" aria-hidden="true">
                      <use xlink:href="#icon-2"></use>
                    </svg>
                    {{item.fileName}}
                  ...
                  <!-- 压缩文件 -->
                  <span v-else-if="item.isArchive"    :title="$t('m.unzip')" >
                    <span style="display:flex;flex-direction:row; justify-content:space-between;">
                      <span>
                        <svg class="icon" aria-hidden="true" style="font-size:12px;">
                          <use xlink:href="#icon-rar"></use>
                        </svg> &nbsp;
                        <span v-if="!item.decompressInfos.length" @click="unzip(item.fileName,index)">{{item.fileName}}</span>
                        <span v-else>{{item.fileName}}</span>
                      </span>
                      <!-- <span v-show="item.isUnzipVisible" class="tip-user"> -->
                      <span v-show="item.decompressInfos.length" >
                        {{$t('m.store.unzippingFile')}}
                  ...
                  <!-- 其他文件 -->
                  <span v-else @click="viewFile(item)">
               ...
               <td class="operation" v-loading="item.loading">                  
                  <!-- download -->
                  <a  :href="'/api/storage/volumes/'+volumeName+'/dirs/'+userFolder+'/download?path='+path+item.fileName+'&isDir='+item.directory+'&type='+type"   class="el-button el-button--text">
                    <i class="el-icon-download"></i>{{$t('m.download')}}
                  </a>  
                  
                  <!-- copy -->
                  <el-button type="text" @click="moveOrCopy='copy';moveOrCopyIndex=index;dialogStrorageTreeVisible=true">
                    <svg class="icon" aria-hidden="true" >
                  ...
                  <!-- move -->
                  <el-button type="text" @click="moveOrCopy='move';moveOrCopyIndex=index;dialogStrorageTreeVisible=true" icon="el-icon-rank">
                    {{$t('m.move')}}
                  </el-button>
                  ...
                  <el-popover
                    ...
                    <el-button type="text" icon="el-icon-view" @click="viewDetailHandle(item.fileName, index)" slot="reference">
                    {{$t('m.size')}} </el-button>
                  </el-popover>
                  ...
                  <span v-if="role==='viewer'">
                    ...
                    <el-button v-if="item.share===0||item.share===1"  type="text" icon="el-icon-setting" @click="shareHandle(index,item.directory,item.share,item.id)" >{{$t('m.setShare')}}</el-button>
                    <el-popover
                        placement="bottom"
                        :title="$t('m.tip')"
                        width="300"
                      ...
                      <el-button v-if="item.share===3"  type="text" icon="el-icon-setting"  slot="reference">{{$t('m.setShare')}}</el-button>
                    </el-popover>

                    <!-- 取消共享 -->
                    <el-button v-if="item.share===1"  type="text" icon="el-icon-circle-close-outline" @click="shareCancelHandle(index,item.id)" >{{$t('m.cancelSharing')}}</el-button>
                  </span>
...
export default {
  ...
  created() {
    ...
    // this.checkIsUploadingFile(); // 查看是否有正在上传的文件
  ...
  methods: {
    ...
    confirmHandle: function() {
      ...
      if(this.moveOrCopy === 'move'){ // 移动
        url = Lib.API(this.volumeName, this.userFolder).storage.moveTo + `?srcPathStr=${this.path+item.fileName}&destPathStr=${this.choosePath}&isDir=${item.directory}&type=${this.type}&id=${item.id}&share=${item.share}`;
        succMess = this.$t('m.move');
      }else if(this.moveOrCopy === 'copy'){　// 复制
        url = Lib.API(this.volumeName, this.userFolder).storage.copyTo + `?srcPathStr=${this.path+item.fileName}&destPathStr=${this.choosePath}&isDir=${item.directory}&type=${this.type}`;
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (data.success) {
            this.$message({
              showClose: true,
              message: succMess + ' ' +  this.$t("m.successAdv"),
              type: "success",
              duration: Lib.Common.messageDuration
            });
            this.viewerGetData();
          ...
          .then(res => {
            ...
            if (data.success) {
              ...
              for(let v of data.shareObjs){
                // response -> shareObjs: [{shareRange: 'all',shareTo: ['all']}];
                // response -> shareObjs: [{shareRange: 'person',shareTo: ['xxx']}];
                // response -> shareObjs: [{shareRange: 'group',shareTo: ['gg']}];
                // response -> shareObjs: [{shareRange: 'group',shareTo: ['gg']},{shareRange: 'person',shareTo: ['xxx']}];
                if(v.shareRange === 'all'){
    ...
    groupPersonFn( arg){
      ...
      this.$axios.post(
        ...
        {
          sharePath: this.path + this.shareItemFileName,
    ...
    viewDetailHandle(fileName,index){
      ...
      this.list[index].detailLoading = true;  // 提示用户发送请求图标
      this.$axios
      // /api/storage/volumes/${this.volumeName}/dirs/${this.userFolder}/detail?path=${path}
      .get(Lib.API(this.volumeName, this.userFolder).storage.viewerViewDetail + `?path=${this.path+fileName}&type=${this.type}`)
      .then(res => {
        const data = res.data;
        if (data.success) {
    ...
    viewerGetData(fileName,dirLsitIndex,interval,refresh){  
      ...
      this.$axios
        ...
        .get(Lib.API(this.volumeName, this.userFolder).storage.viewerGetFileList + `?path=${this.path.length===1?'/':this.path.slice(0,-1)}&type=${this.type}`) 
        .then(res => {
          const data = res.data;
          let time2 = new Date();
          console.log('请求结束');
    ...
    postDir(){
      ...
      */
      ...
      this.$axios.post(
        ...
        {"folderInfo": {
            path: this.path+this.createDirInput, // 路径
            attr: this.attr　// 权限
          }
        })     
        .then(res => {
    ...
    deleteHandle(fileName,index){
      ...
      if(fileName){　// 删除单个文件
        ...
        this.list.forEach((ele,index) => {
          if(ele.isCheck){
            let obj = {
              path: this.path + this.list[index].fileName,
              id: this.list[index].id,
              share: this.list[index].share
            }
            delePara.push(obj);
        ...
        .then(res => {
          ...
          if (data.success) { 
            if(fileName){ // 删除一个情况
              ...
              this.$message({
                showClose: true,
                message: fileName + " " + this.$t("m.deleted"),
                type: "success",
                duration: Lib.Common.messageDuration
              });
    ...
    viewFile(item,isImage){
      ...
      .get(Lib.API(this.volumeName, this.userFolder).storage.viewContent + `?path=${this.path+ item.fileName}&type=${this.type}`)
      .then( res => {
        const data = res.data;
        if(data.success){
    ...
    cancelUpload(){
      ...
      this.uploadingDiv = false;
      // this.uploadingPath
      // document.querySelector('input[type="file"]').outerHTML = document.querySelector('input[type="file"]').outerHTML;
    },
    // 暂停上传
    stopUpload(){ 
      this.isPause = true;
    ...
    uploadChange(first){
      this.isPause = false;
      // console.log('enter uploadChange');
      ...
      console.log(fileSize/step,',sliceTotal = '+sliceTotal);
      console.log('this.sliceCurrent = ' + this.sliceCurrent);
      ...
      var url = Lib.API(this.volumeName, this.userFolder).storage.upload + `?path=${path.length===1?path:path.slice(0,-1)}&type=${this.type}&sameUpload=${sameUpload}&fileSize=${file.size}&sliceCurrent=${this.sliceCurrent}&fileName=${this.uploadingFileName}`
    ...
    unzip(fileName,index){
      this.list[index].isUnzipingLoading = true;
     
      this.$axios
      .get(Lib.API(this.volumeName, this.userFolder).storage.unzip + `?path=${this.userFolder+(this.path.length===1?this.path:this.path.slice(0,-1))}&fileName=${fileName}&type=${this.type}`)
      .then(res => {
        const data = res.data;
        if (data.success) {
          // this.list[index].isUnzipVisible = false;
          // this.viewerGetData(); // 更新列表
          this.unzipInterval(index);
        }else{
          // this.list[index].isUnzipVisible = false;
          this.list[index].isUnzipingLoading = false;
          var messageCode = data.message || "default";
          var messageText = this.$t("m.responseErrorMessage." + messageCode);
          Lib.Common.errorMessage(messageText,data.messageParams);
        }
    ...
    // 解压 过程 定时器
    unzipInterval(index){
      // /api/storage/volumes/{volumeName}/dirs/{userFolder}/decompressInfo
      clearInterval(this.timerIdUnzip); // 关闭定时器
      this.timerIdUnzip = setInterval(() => {
        this.$axios
          .get(Lib.API(this.volumeName, this.userFolder).storage.unzipInterval + `?path=${this.userFolder+(this.path.length===1?this.path:this.path.slice(0,-1))}&type=${this.type}`)
          .then(res => {
