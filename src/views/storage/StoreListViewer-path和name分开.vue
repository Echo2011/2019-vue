           ...
           <tbody v-if="list.length>0" class="tbody-viewer" >
             <tr v-for="(item,index) in list" :key="index" >
               ...
               <td class="operation" v-loading="item.loading">  
                  ...
                  <!-- download -->                   
                  <a  :href="'/api/storage/volumes/'+volumeName+'/dirs/'+userFolder+'/download?path='+path+item.fileName+'&isDir='+item.directory+'&type='+type" >
                  ...
                  <span v-if="role==='viewer'">
                    ...
                    <el-popover
                      ...
                      <el-button :disabled="Boolean(item.decompressInfos.length)" :class="{btnDisabled:Boolean(item.decompressInfos.length)}" v-if="item.share===3"  type="text" icon="el-icon-setting"  slot="reference">{{$t('m.setShare')}}</el-button>
                    </el-popover>
