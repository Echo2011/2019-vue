  ...
  <section class="show-info viewer">
    ...
    <table v-loading="loading" :element-loading-text="$t('m.loading')">
        ...
        <tbody v-if="list.length>0" class="tbody-viewer" >
          <tr v-for="(item,index) in list" :key="item.fileName" >
            <td>
            ...
            <td class="operation" v-loading="item.loading" element-loading-text="">
              ...
              <span v-if="publicOrShare==='share'&&dirList.length===1">
                <a :href="'/api/storage/volumes/'+item.ownerVolumeName+'/dirs/'+item.ownerUserFolder+'/download?path='+item.sharePath+'&isDir='+item.directory+'&type='+type"  class="el-button el-button--text">
                  <i class="el-icon-download"></i>{{$t('m.download')}}
                </a>
              </span>
              <span v-else-if="publicOrShare==='public'"> <!-- 公共数据　-->
                <a  :href="'/api/storage/volumes/public/dirs/common/download?path='+path+item.fileName+'&isDir='+item.directory+'&type='+type"  class="el-button el-button--text">
                  <i class="el-icon-download"></i>{{$t('m.download')}}
                </a>
              </span>
              <span v-else> <!-- 共享Ｎ级目录下 -->
                <a  :href="'/api/storage/volumes/'+ownerVolumeName+'/dirs/'+ownerUserFolder+'/download?path='+path+item.fileName+'&isDir='+item.directory+'&type='+type"  class="el-button el-button--text">
                  <i class="el-icon-download"></i>{{$t('m.download')}}
                </a>
              </span>

              <!-- copy icon -->
              <el-button type="text" @click="copyIndex=index;dialogStrorageTreeVisible=true">
                <svg class="icon" aria-hidden="true" >
    ...
    path: function () {
      if(this.publicOrShare === 'public'){
        ...
        if(this.dirList.length === 1){  
          ...
          return this.ownerPath + '/'+this.dirList.slice(1).join('/')+'/'; // ["全部文件",'xx','yy','zz'] => /aa/bb/xx/yy/zz/
        }
      }
  ...
  methods: {
    ...
    confirmHandle() {
      ...
      if(this.publicOrShare==='public'){
        ownerVolume = 'public';
        owner = 'common';
      }else if(this.publicOrShare==='share'&&this.dirList.length===1){
      ...
      srcPathStr = (this.publicOrShare==='share'&&this.dirList.length===1) ? this.list[this.copyIndex].sharePath : (this.path+fileName);

      url = Lib.API(this.volumeName, this.userFolder).storage.shareCopy + `?srcPathStr=${srcPathStr}&destPathStr=${this.choosePath}&isDir=${this.list[this.copyIndex].directory}&type=${this.type}&ownerVolume=${ownerVolume}&owner=${owner}`;
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (data.success) {
            this.$message({
              showClose: true,
              message: this.$t('m.copy') + ' ' +  this.$t("m.successAdv"),
              type: "success",
              duration: Lib.Common.messageDuration
            });
    ...
    viewDetailHandle(fileName,index){
      ...
      if(this.publicOrShare == 'share' && (this.dirList.length===1)){
        ...
        url = Lib.API(this.list[index].volumeName, this.list[index].owner).storage.viewerViewDetail + `?path=${this.list[index].sharePath}&type=${this.type}`; 
      }else if(this.publicOrShare == 'public' ){
        url = Lib.API('public', 'common').storage.viewerViewDetail + `?path=${this.path+fileName}&type=${this.type}`;
      }else {
        url = Lib.API(this.ownerVolumeName, this.ownerUserFolder).storage.viewerViewDetail + `?path=${this.path+fileName}&type=${this.type}`;
      }
    ...
    viewerGetData(fileName,dirLsitIndex){   
      ...
      if(this.publicOrShare === 'public'){ // 公共数据
        url = Lib.API(this.volumeName, this.userFolder).storage.viewerGetFileList + `?path=${this.path.length===1?'/':this.path.slice(0,-1)}&type=${this.type}`;
      }else if(this.publicOrShare === 'share'){ // 获取分享到自己的数据
      // return false;
        if(this.dirList.length > 1) { // 获取 n >= 2级目录
          url = Lib.API(this.ownerVolumeName, this.ownerUserFolder).storage.viewerGetFileList + `?path=${this.path.length===1?'/':this.path.slice(0,-1)}&type=${this.type}`;
        }else{　// 获取一级目录
    ...
    viewFile(fileName,index,isImage){
      ...
      var path = '', url = '';
      if(this.publicOrShare == 'share' && (this.dirList.length===1)){
        url = Lib.API(this.list[index].volumeName, this.list[index].owner).storage.viewContent + `?path=${this.list[index].sharePath}&type=${this.type}`; 
      }else if(this.publicOrShare == 'public' ){
        url = Lib.API('public', 'common').storage.viewContent + `?path=${this.path+fileName}&type=${this.type}`;
      }else {
        url = Lib.API(this.ownerVolumeName, this.ownerUserFolder).storage.viewContent + `?path=${this.path+fileName}&type=${this.type}`;
      }
      ...
      this.$axios
      // Lib.API(this.volumeName, this.userFolder).storage.viewContent + `?path=${path}`
      .get(url)
      .then( res => {
        const data = res.data;
        if(data.success){
